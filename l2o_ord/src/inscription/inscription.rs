...
use crate::inscription::envelope;
...
impl Inscription {
    ...
    pub fn content_encoding(&self) -> Option<HeaderValue> {
        HeaderValue::from_str(
            core::str::from_utf8(self.content_encoding.as_ref()?).unwrap_or_default(),
        )
        .ok()
    }
    ...
    pub fn metaprotocol(&self) -> Option<&str> {
        core::str::from_utf8(self.metaprotocol.as_ref()?).ok()
    }
...
mod tests {
    ...
    fn pointer_value() {
        ...
        let inscription =
            Inscription::from_file(Chain::Mainnet, file.path(), None, None, None, None, false)
                .unwrap();
