...
use crate::COIN_VALUE;
use crate::CYCLE_EPOCHS;
...
impl Sat {
        ...
        for c in s.chars() {
            match c {
                'a'..='z' => {
                    ...
                    if x > Self::SUPPLY {
                        anyhow::bail!("sat name out of range");
                    }
                }
                _ => anyhow::bail!("invalid character in sat name: {c}"),
            }
        }
        Ok(Sat(Self::SUPPLY - x))
    }

    fn from_degree(degree: &str) -> anyhow::Result<Self> {
        let (cycle_number, rest) = degree
            .split_once('°')
            .ok_or_else(|| anyhow::anyhow!("missing degree symbol"))?;
        let cycle_number = cycle_number.parse::<u32>()?;

        let (epoch_offset, rest) = rest
            .split_once('′')
            .ok_or_else(|| anyhow::anyhow!("missing minute symbol"))?;
        let epoch_offset = epoch_offset.parse::<u32>()?;
        if epoch_offset >= SUBSIDY_HALVING_INTERVAL {
            anyhow::bail!("invalid epoch offset");
        }

        let (period_offset, rest) = rest
            .split_once('″')
            .ok_or_else(|| anyhow::anyhow!("missing second symbol"))?;
        let period_offset = period_offset.parse::<u32>()?;
        if period_offset >= DIFFCHANGE_INTERVAL {
            anyhow::bail!("invalid period offset");
        }
        ...
        if !rest.is_empty() {
            anyhow::bail!("trailing characters");
        }

        if block_offset >= height.subsidy() {
            anyhow::bail!("invalid block offset");
        }

        Ok(height.starting_sat() + block_offset)
    }

    fn from_decimal(decimal: &str) -> anyhow::Result<Self> {
    ...
    fn from_percentile(percentile: &str) -> anyhow::Result<Self> {
        if !percentile.ends_with('%') {
            anyhow::bail!("invalid percentile: {}", percentile);
        }
